    ...
    <div className="bg-background">
      <main className="mx-auto w-full max-w-7xl px-6 pb-16 pt-10 sm:pb-24 lg:px-8">
        ...
        <div className="mx-auto mt-10 max-w-2xl text-center">
          <p className="text-xl font-black leading-8">404</p>
          <h1 className="mt-4 text-2xl font-bold tracking-tight sm:text-3xl">
            {sharedNotFoundConfig.title}
          </h1>
          <p className="mt-4 text-base leading-7 text-foreground/70 sm:mt-6 sm:text-lg sm:leading-8">
        ...
        <div className="mx-auto mt-5 flow-root max-w-lg sm:mt-10">
          ...
          <ul role="list" className="divide-y">
            {mainCategoryConfig.map((category) => (
              <Link key={v4()} href={`/category/${category.slug}` || ""}>
                <li className="relative flex gap-x-6 border-b border-border/50 py-6">
                  <div className="flex h-10 w-10 flex-none items-center justify-center rounded-lg shadow-sm">
                    <category.icon className="h-6 w-6" />
                  </div>
                  <div className="my-auto flex-auto items-center text-sm font-semibold leading-6">
                    {category.title}
                  </div>
